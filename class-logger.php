...
class WPSS_Logger {
	const LOGFILE = 'wpss-backup-log.txt';
	...
	public function get_log() {
		...
		if (!$wp_filesystem) {
			...
			if (empty($wp_filesystem)) {
				send_response_wpss('FS_INIT_FAILED-010');
				return false;
			}
		}
	...
	public function delete_log() {
		...
		if (!$wp_filesystem) {
			initiate_filesystem_wpss();
			if (empty($wp_filesystem)) {
				send_response_wpss('FS_INIT_FAILED-011');
				return false;
			}
		}
		$this->logFile = null;
	...
	public function get_log_file() {
		if (!$this->logFile) {
			...
			if (isset($files[0])) {
				...
				$this->logFile = $path . '.' . WPSS_Factory::secret(self::LOGFILE);
			}
		}
		return $this->logFile;
	}
	...
	public function set_log_now($type, $msg, $action_id, $data, $show_user) {
		...
		$result = $wpdb->insert($wpdb->base_prefix . 'wpss_activity_log', array('type' => $type, 'log_data' => $LogData, 'action_id' => $action_id, 'show_user' => $show_user));
	}
}
