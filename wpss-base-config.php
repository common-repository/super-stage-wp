...
abstract class WPSS_Base_Config {
	protected $config;
	...
	public function get_option($option_name = null) {
		if (array_key_exists($option_name, $this->used_options)) {
			return WPSS_Factory::get('config')->get_option($option_name);
		}
		wpss_log($option_name, "--------not registered WPSS_Base_Config--------");
	}

	public function set_option($option_name, $val = null) {
		if (array_key_exists($option_name, $this->used_options)) {
			return WPSS_Factory::get('config')->set_option($option_name, $val);
		}
		wpss_log($option_name, "--------not registered WPSS_Base_Config--------");
	}
	...
	public function flush() {
		foreach ($this->used_options as $option_name => $v) {
			if ($v == 'flushable') {
				WPSS_Factory::get('config')->set_option($option_name, false);
			}
		}
