...
//			this._fixSelectionState(node);
	...
	treeLoad: function(ctx, source) {
		if (typeof wpss_fancy_tree_time != 'undefined' && wpss_fancy_tree_time + 10 > jQuery.now() ) {
			if (wpss_fancy_tree_request === ctx.options.source.data.action) {
				// console.log('WPSS skip multiple request', wpss_fancy_tree_request);
				return ;
			}
		}
		wpss_fancy_tree_request = ctx.options.source.data.action;
		wpss_fancy_tree_time = jQuery.now();
