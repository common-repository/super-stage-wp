...
class WPSS_DatabaseBackup {
	...
	public function __construct($processed = null) {
		$this->database = WPSS_Factory::db();
		$this->config = WPSS_Factory::get('config');
		$this->processed = $processed ? $processed : new WPSS_Processed_iterator();
		$this->exclude_class_obj = WPSS_Base_Factory::get('WPSS_ExcludeOption');
	...
	public function get_file() {
		$file = apply_filters('get_realtime_partial_db_file_wpss', '');

		wpss_log($file,'----------get_file-$file----------------');
		...
		if (wpss_is_meta_data_backup()) {
			$file = rtrim($this->config->get_backup_dir(), '/') . '/' . DB_NAME . "-wpss_meta.sql";
		} else {
		...
		$prepared_file_name = $file . '.' . WPSS_Factory::secret(DB_NAME);
	...
	private function write_db_dump_header() {
		...
		if($this->config->choose_db_backup_path() === false){
			...
			$msg = sprintf(__("A database backup cannot be created because WordPress does not have write access to '%s', please ensure this directory has write access.", 'wpss'), $get_default_backup_dir);
				WPSS_Factory::get('logger')->log($msg, 'backups', wpss_get_cookie('backupID'));
				return false;
		}
		...
		$this->write_to_temp("-- Super Stage WP SQL Dump\n");
		$this->write_to_temp("-- Version " . WPSS_VERSION . "\n");
		$this->write_to_temp("-- https://wpsuperstage.com\n");
...
/*!40111 SET @OLD_SQL_NOTES=@@SQL_NOTES, SQL_NOTES=0 */;\n\n");
	...
	public function execute() {
		...
		if (apply_filters('is_realtime_partial_db_backup_wpss', '')) {
			$this->processed->update_iterator('header', -1);
		}
		...
		while ( $table_meta ) {
			...
			if ($table_meta->offset > 1) {
				WPSS_Factory::get('logger')->log(sprintf(__("Resuming table '%s' at row %s.", 'wpss'), $table_meta->name, $table_meta->offset), 'backups', $backup_id);
			}
			...
			WPSS_Factory::get('logger')->log(sprintf(__("Processed table %s.", 'wpss'), $table_meta->name), 'backups', $backup_id);
...
/*!40111 SET SQL_NOTES=@OLD_SQL_NOTES */;\n\n");
	...
	public function collect_tables_for_backup(){
		...
		if($this->config->get_option('collected_tables_for_backups')){
			...
			wpss_log('', "--------skipping collect_tables_for_backup--------");
		...
		if (apply_filters('is_realtime_partial_db_backup_wpss', '')) {
			$tables = apply_filters('get_realtime_full_backup_needed_tables_wpss', time());
		...
		foreach ($tables as $table) {
			...
			// wpss_log($exclude_status, '---------------$exclude_status-----------------');

			if ($exclude_status === 'table_excluded') {
				continue;
			}

			// wpss_log($table , '---------------Table not completed-----------------');
	...
	private function is_already_inserted($table){
		$qry = "SELECT name FROM " . $this->database->base_prefix . "wpss_processed_iterator WHERE name = '" . $table . "'";
	...
	private function save_colllected_tables(){
		...
		$sql = "insert into " . $this->database->base_prefix . "wpss_processed_iterator (`id`, `name`, `offset`) values $this->bulk_table_insert";
		wpss_log($sql,'-----------$sql save_colllected_tables----------------');
		$result = $this->database->query($sql);
		wpss_log($result,'-----------$result save_colllected_tables----------------');
	}

	public function backup_database_table($table, $offset, $table_skip_status) {

		wpss_manual_debug('', 'start_backup_' . $table);

		$db_error = __('Error while accessing database.', 'wpss');
...
/*!40101 SET character_set_client = utf8 */;\n";
			...
			if ($table_create === false) {
				...
				wpss_log($this->database->last_error, '-----dbb_error---');
		...
		if ($table_count != 0) {
			for ($i = $offset; $i < $table_count; $i = $i + $this->select_query_limit) {
				...
				wpss_manual_debug('', 'during_db_backup', 1000);
				...
				if ($table_data === false || !is_array($table_data[0])) {
					...
					wpss_log($this->database->last_error, '-----dbb_error-2--');
				...
				wpss_log($table . ' - ' . $row_count, '---------Table backing up-----------------------');

				if($this->app_functions->is_backup_request_timeout($return = true, true)){
					$this->processed->update_iterator($table, $row_count);
					send_response_wpss('Backing up table' . $table . ' (Offset : ' .$row_count . ')' );
				}
	...
	private function backup_trigger_table(&$tables){
		...
		$table = WPSS_Base_Factory::get('Trigger_Init')->get_trigger_query_tablename();
		wpss_log($table,'-----------$table----------------');
		...
		WPSS_Base_Factory::get('Trigger_Init')->truncate_table();
	...
	public function backup_specific_column_of_table($table, $column){
		...
		if ($this->processed->is_complete($table)) {
			wpss_log(array(),'-----------table alreay completed----------------');
			// return true;
		}
		...
		wpss_log($offset,'-----------$offset----------------');

		$table_count = $this->database->get_var("SELECT COUNT(*) FROM $table");

		wpss_log($table_count,'-----------$table_count----------------');

		if ($offset == 0 && $table_count != 0) {
			wpss_log(array(),'-----------First request----------------');
	...
	*/
	...
	public function shell_db_dump(){
		...
		wpss_log('', "----trying----shell_db_dump----backup----");
	...
	private function check_is_shell_db_dump_running(){
		...
		wpss_log($filesize, '---------------$filesize-----------------');
		wpss_log($this->config->get_option('shell_db_dump_prev_size'), '---------------$prev-----------------');
	...
	private function backup_db_dump() {
		...
		$filesize = wpss_get_file_size($file);
	...
	private function exec_mysqldump($tables, $structure_only = ''){
		...
		wpss_log($command, '---------------$command-----------------');
		return $this->wpss_exec($command);
	}
	...
	private function check_mysql_paths() {
		...
		if (substr(PHP_OS, 0, 3) == 'WIN') {
			...
			$paths['mysql'] = $this->wpss_exec('which mysql', true);
			if (empty($paths['mysql']))
				$paths['mysql'] = 'mysql'; // try anyway

			$paths['mysqldump'] = $this->wpss_exec('which mysqldump', true);
	...
	private function wpss_exec($command, $string = false, $rawreturn = false) {
		if ($command == '')
			return false;
	...
	private function write_to_temp($out) {
		...
		if (fwrite($this->temp, $out) === false) {
			throw new Exception(__('Sql Backup : Error writing to php://memory.', 'wpss'));
		}
	}
	...
	private function persist() {
		...
		if (!$fh) {
			throw new Exception(__('Sql Backup : Error creating sql dump file.', 'wpss'));
		}
		...
		if (!fclose($fh)) {
			throw new Exception(__(' Sql Backup : Error closing sql dump file.', 'wpss'));
		}

		if (!fclose($this->temp)) {
			throw new Exception(__(' Sql Backup : Error closing php://memory.', 'wpss'));
		}
	...
	public function compress(){
		wpss_log(func_get_args(), "--------" . __FUNCTION__ . "--------");
		if(!wpss_function_exist('gzwrite') || !wpss_function_exist('gzopen') || !wpss_function_exist('gzclose') ){
			wpss_log(array(), '--------ZGIP not available--------');
	...
	private function gz_compress_file($source, $offset, $level = 9){
		...
		if (filesize($source) < 5 ) {
			wpss_log(array(),'-----------FILE contains nothing so delete it and skip compression----------------');
			return @unlink($source);
		}

		wpss_log(func_get_args(), "--------" . __FUNCTION__ . "--------");
		wpss_log(filesize($source),'-----------filesize($source)----------------');
		...
		while (!feof($fp_in)){
			...
			wpss_manual_debug('', 'during_compress_db', 10);
		...
		if ($break) {
			...
			send_response_wpss('Compressing database file, Offset : ' .$offset );
		}
	...
	public function encrypt($fullpath = false) {
		...
		wpss_log($is_enabled,'-----------$is_enabled----------------');
		...
		wpss_log($fullpath,'-----------$fullpath----------------');

		if (!function_exists('mcrypt_encrypt') && !extension_loaded('openssl')) {
			WPSS_Factory::get('logger')->log('Your web-server does not have the PHP/mcrypt / PHP/OpenSSL module installed. Without it, encryption will be a lot slower.', 'backups', wpss_get_cookie('backupID'));
		}

		// include Rijndael library from phpseclib
		$ensure_phpseclib = wpss_ensure_phpseclib('Crypt_Rijndael', 'Crypt/Rijndael');

		if (is_wp_error($ensure_phpseclib)) {
			WPSS_Factory::get('logger')->log("Failed to load phpseclib classes (" . $ensure_phpseclib->get_error_code() . "): " . $ensure_phpseclib->get_error_message(), 'backups', wpss_get_cookie('backupID'));
			return false;
		}

		// open file to read
		if (false === ($file_handle = fopen($fullpath, 'rb'))) {
			WPSS_Factory::get('logger')->log("Failed to open file for read access: $fullpath", 'backups', wpss_get_cookie('backupID'));
			return false;
		}
		...
		if (file_exists($encrypted_path)) {
			WPSS_Factory::get('logger')->log("Temporary encryption file found, will try to resume the encryption", 'backups', wpss_get_cookie('backupID'));
			...
			$true_data_encrypted = $data_encrypted - ($data_encrypted % WPSS_CRYPT_BUFFER_SIZE);

			if ($data_encrypted >= $block_length) {

				// Open existing file from the path
				if (false === ($encrypted_handle = fopen($encrypted_path, 'rb+'))) {
					WPSS_Factory::get('logger')->log("Failed to open file for write access on resumption: $encrypted_path", 'backups', wpss_get_cookie('backupID'));
					$resumption = false;
				}

				// First check if our buffer size needs padding if it does increase buffer size to length that doesn't need padding
				if (0 != WPSS_CRYPT_BUFFER_SIZE % 16) {
					$pad = 16 - (WPSS_CRYPT_BUFFER_SIZE % 16);
					$true_buffer_size = WPSS_CRYPT_BUFFER_SIZE + $pad;
				} else {
					$true_buffer_size = WPSS_CRYPT_BUFFER_SIZE;
				}
				...
				if (-1 === (fseek($encrypted_handle, $data_encrypted - ($block_length + $partial_data_size)))) {
					WPSS_Factory::get('logger')->log("Failed to move file pointer to correct position to get IV: $encrypted_path", 'backups', wpss_get_cookie('backupID'));
					$resumption = false;
				}

				// Read previous block length from file
				if (false === ($iv = fread($encrypted_handle, $block_length))) {
					WPSS_Factory::get('logger')->log("Failed to read from file to get IV: $encrypted_path", 'backups', wpss_get_cookie('backupID'));
					$resumption = false;
				}
				...
				if (-1 === (fseek($file_handle, $true_data_encrypted))) {
					WPSS_Factory::get('logger')->log("Failed to move file pointer to correct position to resume encryption: $fullpath", 'backups', wpss_get_cookie('backupID'));
					$resumption = false;
				}
		...
		if (!$resumption) {
			// WPSS_Factory::get('logger')->log("Could not resume the encryption will now try to start again", 'backups', wpss_get_cookie('backupID'));
			...
			if (-1 === (fseek($file_handle, 0))) {
				WPSS_Factory::get('logger')->log("Failed to move file pointer to start position to restart encryption: $fullpath", 'backups', wpss_get_cookie('backupID'));
				$resumption = false;
			}
		}
		...
		if (!$resumption) {
			...
			if (false === ($encrypted_handle = fopen($encrypted_path, 'wb+'))) {
				WPSS_Factory::get('logger')->log("Failed to open file for write access: $encrypted_path", 'backups', wpss_get_cookie('backupID'));
				return false;
			}
		}
		...
		while ($data_encrypted < $file_size) {
			...
			if (false === ($file_part = fread($file_handle, WPSS_CRYPT_BUFFER_SIZE))) {
				WPSS_Factory::get('logger')->log("Failed to read from file: $fullpath", 'backups', wpss_get_cookie('backupID'));
				return false;
			}
			...
			if (false === fwrite($encrypted_handle, $encrypted_data)) {
				WPSS_Factory::get('logger')->log("Failed to write to file: $encrypted_path", 'backups', wpss_get_cookie('backupID'));
				return false;
			}

			$data_encrypted += WPSS_CRYPT_BUFFER_SIZE;

			$time_since_last_logged = time() - $time_last_logged;
			if ($time_since_last_logged > 5) {
				$time_since_last_logged = time();
				WPSS_Factory::get('logger')->log("Encrypting file: completed $data_encrypted bytes", 'backups', wpss_get_cookie('backupID'));
			}
		...
		if ($break) {
			send_response_wpss('Encrypting database file, Offset : ');
		}
		...
		if (false === rename($encrypted_path, $result_path)) {
			WPSS_Factory::get('logger')->log("File rename failed: $encrypted_path -> $result_path", 'backups', wpss_get_cookie('backupID'));
			return false;
		}
	...
	public function is_wp_table($table) {
		...
		$wpss_strpos = stripos($table, $wp_prefix);
	...
	public function complete_all_tables(){
		$update_all_tables = "UPDATE " . $this->database->base_prefix . "wpss_processed_iterator SET `offset` = '-1' WHERE `name` NOT LIKE '%/%'";
		wpss_log($update_all_tables, '---------------$update_all_tables-----------------');
