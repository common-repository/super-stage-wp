...
class WPSS_Stage_To_Live{
	...
	private function choose_action(){
		...
		wpss_log($current_action, '--------$current_action--------');
		...
		switch ($current_action) {
			...
			case 'final_things_to_do';
				...
				break;
			default:
				wpss_log(array(), '--------Something went wrong--------');
				break;
		}
	...
	private function clone_db(){
		wpss_manual_debug('', 'start_clone_db_S2L');
		...
		foreach ($tables as $table) {
			...
			wpss_manual_debug('', 'during_clone_table_S2L_' . $table);
			...
			if( $table_skip_status === 'content_excluded'){
				...
				wpss_log($table,'-----------content excluded so skip now----------------');
				continue;
			}
		...
		wpss_manual_debug('', 'end_clone_db_S2L');
	...
	private function replace_links(){
		wpss_manual_debug('', 'start_replace_links_S2L');

		$this->replace_deep_links();

		wpss_manual_debug('', 'start_replace_site_links_S2L');

		$this->replace_site_links();

		wpss_manual_debug('', 'end_replace_site_links_S2L');

		wpss_manual_debug('', 'end_replace_links_S2L');
	...
	private function replace_site_links(){
		...
		if (!$result) {
			...
			wpss_log('Replacing site url has been failed. ' . $error, '--------FAILED----------');
		} else {
			// $this->logger->log('Replacing siteurl has been done succesfully', 'staging', $this->staging_id);
			wpss_log('Replacing siteurl has been done succesfully', '--------SUCCESS----------');
		}
		...
		if (!$result) {
			...
			wpss_log("Updating option [rewrite_rules] has been done succesfully", '--------SUCCESS-------------');
		}
		...
		if (!$result) {
			...
			wpss_log("User roles modification has been failed", '--------FAILED-------------');
		} else {
			// $this->logger->log('User roles has been modified succesfully', 'staging', $this->staging_id);
			wpss_log("User roles has been modified succesfully", '--------SUCCESS-------------');
		}
		...
		wpss_log($usermeta_sql, '--------$usermeta_sql--------');

		$result_usermeta = $this->wpdb->query( $usermeta_sql );

		wpss_log($result_usermeta, '--------$result_usermeta--------');
		...
		wpss_log($options_sql, '--------$options_sql--------');
		wpss_log($result_options, '--------$result_options--------');
		...
		if ($result_options === false || $result_usermeta === false) {
			...
			wpss_log('Updating db prefix "' . $this->wpdb->base_prefix . '" to  "' . $new_prefix . '" has been done succesfully.', '--SUCCESS-------------------');
		}
	...
	private function get_folders(){
		wpss_manual_debug('', 'start_get_folders_S2L');

		$this->file_iterator->get_folders();

		wpss_manual_debug('', 'end_get_folders_S2L');
	...
	private function get_hash_by_folders(){
		wpss_manual_debug('', 'start_get_hash_by_folder_S2L');
		...
		while(!$break){
			...
			$path = wpss_add_fullpath($relative_path);
		...
		wpss_manual_debug('', 'end_get_hash_by_folder_S2L');
	...
	public function process_file($iterator, $is_recursive, $path, &$counter, $iterator_loop_limit, &$query, $key) {
		wpss_manual_debug('', 'during_process_file_S2L', 500);
		...
		if(!$this->is_file_modified($file)){
			// wpss_log(array(), '--------SKIP FILE 2--------');
			$this->app_functions->check_timeout_iter_file($path, $counter, $iterator_loop_limit, $query, $key);
			return ;
		}

		wpss_log($file, '--------File Modified--------');
	...
	private function get_hash_dir($path, $offset, &$temp_counter, $deep_dirs){
		...
		$seek_file_iterator = new WPSS_Seek_Iterator($this, $type = "STAGING_TO_LIVE", 500, $category = 'staging');
	...
	private function convert_staging_to_live($path){
		if (wpss_is_wp_content_path($path) && wpss_is_wp_content_dir_moved_outside_root($this->config) ) {
	...
	private function is_hash_modified($stage_file, $live_file){
		...
		if(!wpss_is_hash_required($stage_file)){
			wpss_log(array(), '---------cannot find has so checking is_size_mismatch------------');
			return $this->is_size_mismatch($stage_file, $live_file);
		}

		$stage_file_hash = wpss_get_hash($stage_file);
		$live_file_hash  = wpss_get_hash($live_file);
	...
	private function is_wpss_dir($file){
		if(stripos($file, WPSS_PLUGIN_DIR) !== false){
			return true;
		}
	...
	private function get_queued_files(){
		...
		$limit = empty($limit) ? WPSS_STAGING_DEFAULT_FILE_COPY_LIMIT : $limit;

		$sql = "SELECT file_path FROM `".$this->wpdb->base_prefix."wpss_current_process` WHERE `status` = 'Q' ORDER BY `id` LIMIT " . $this->copy_file_offset . ", " . $limit . "";

		return $this->wpdb->get_results($sql, ARRAY_N);
	}

	private function copy_files_to_live(){
		wpss_manual_debug('', 'start_copy_files_S2L');
		...
		wpss_log($queue_files, '--------$queue_files--------');
		...
		foreach ($queue_files as $key => $file) {
			...
			wpss_manual_debug('', 'start_copy_files_S2L', 500);

			$counter++ ;
			$stage_file = $file[0];

			$stage_file = wpss_add_fullpath($stage_file);

			wpss_log($this->stage_path, '--------$this->stage_path--------');
			wpss_log($this->live_path, '--------$this->live_path--------');

			$live_file = $this->convert_staging_to_live($stage_file);

			wpss_log($stage_file, '--------$file--------');
			wpss_log($live_file, '--------$live_file--------');
			...
			if (!file_exists($live_dir)) {
				wpss_log($live_dir, '--------Could not create folder - --------');
					continue;
			}

			if(filesize($stage_file) < WPSS_STAGING_COPY_SIZE){

				$copy_status = $this->filesystem->copy($stage_file, $live_file, true, FS_CHMOD_FILE);
				wpss_log($copy_status, '--------$copy_status--------');
				if (!$copy_status){
					wpss_log(array(), '--------Could not copy this file--------');
					wpss_log(error_get_last(), '---------------error_get_last()-----------------');
				}

			} else {
				$copy_status = wpss_copy_large_file($stage_file, $live_file);
				wpss_log($copy_status, '--------$copy_status--------');
				if (!$copy_status){
					wpss_log(array(), '--------Could not copy this file--------');
					wpss_log(error_get_last(), '---------------error_get_last()-----------------');
		...
		wpss_log($counter, '--------$counter--------');
		wpss_log($this->copy_file_offset, '--------$this->copy_file_offset--------');

		wpss_manual_debug('', 'end_copy_files_S2L');
	...
	private function final_things_to_do(){
		...
		wpss_manual_debug('', 'start_final_things_todo_S2L');
		$new_prefix = $this->config->get_option('s2l_live_db_prefix');
		...
		wpss_manual_debug('', 'end_final_things_todo_S2L');

		$this->completed();

		wpss_manual_debug('', 'completed_S2L');
	...
	private function sub_dir_actions($new_prefix){
		wpss_log(func_get_args(),'-----------sub_dir_actions func_get_args()----------------');
		...
		wpss_log($result,'----------sub_dir_actions -$result----------------');
	...
	private function remove_robot_txt(){
		...
		wpss_log($delete_status, '--------$delete_status--------');
	}
	...
	private function completed(){
		...
		$this->app_functions->run_deactivate_plugin('super-stage-wp-staging/super-stage-wp-staging.php', $live_prefix);
		$this->app_functions->run_activate_plugin('super-stage-wp/super-stage-wp.php', $live_prefix);

		// WPSS_Base_Factory::get('WPSS_Backup_Analytics')->send_report_data($this->staging_id, 'STAGING_TO_LIVE', 'SUCCESS');
	...
	private function flush_all_flags(){
		...
		wpss_manual_debug('', 'flush_flags_S2L');
	}
	...
	private function setup(){
		...
		wpss_log(array(), '--------SETUP STARTED--------');

		// WPSS_Base_Factory::get('WPSS_Backup_Analytics')->send_report_data($this->staging_id, 'STAGING_TO_LIVE', 'STARTED');
	...
	public function check_permissions() {
		...
		die( sprintf ( __('Access denied. You need to <a href="%1$s" target="_blank">Login</a> first','wpss'), get_admin_url() ) );
	}
	...
	private function is_wpss_request(){

		if(empty($_REQUEST['data']) || !empty($_REQUEST['action'])){
			return false;
		}

		$is_wpss_request = @unserialize(base64_decode($_REQUEST['data']));

		wpss_log($is_wpss_request, '---------------$is_wpss_request-----------------');

		if (empty($is_wpss_request) || empty($is_wpss_request['action']) || $is_wpss_request['action'] !== 'update_in_staging' )  {
			return false;
		}
	...
	private function staging_completed(){
		...
		$sql = "SELECT value FROM `".$live_prefix."wpss_options` WHERE `name` = 'same_server_staging_status'";
		wpss_log($sql, '--------$sql--------');
		$status = $this->wpdb->get_var($sql);
		wpss_log($status, '--------$status--------');
	...
	private function is_login_page(){
		...
		if (!$this->config->get_option('staging_login_custom_link')) {
			wpss_log($GLOBALS["_SERVER"]["REQUEST_URI"],'-----------$GLOBALS["_SERVER"]["REQUEST_URI"]----------------');
	...
	public function get_last_time_copy_to_live(){
		...
		return user_formatted_time_wpss($time);
	}
	...
	public function is_allowed_to_copy_site_to_live() {
		return true;
		// $live_prefix = $this->config->get_option('s2l_live_db_prefix');
		// $sql = "SELECT value FROM `" . $live_prefix . "wpss_options` WHERE `name` = 'privileges_args'";
		// $raw_result = $this->wpdb->get_var($sql);

		// $object = json_decode($raw_result);
		// $features = json_decode(json_encode($object), true);

		// $is_allowed = isset($features['WPSS_Staging_To_Live']);

		// wpss_log($is_allowed,'-----------$is_allowed----------------');
