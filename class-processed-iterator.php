...
class WPSS_Processed_iterator extends WPSS_Processed_Base {
	const COMPLETE = -1;
	...
	public function get_table($name) {
		$single_table_result = $this->db->get_results("SELECT * FROM {$this->db->base_prefix}wpss_processed_{$this->getTableName()} WHERE name = '$name'");
	...
	public function get_unfnished_folder() {
		$sql = "SELECT * FROM {$this->db->base_prefix}wpss_processed_{$this->getTableName()} WHERE `offset` != -1 LIMIT 1";
		$response = $this->db->get_results($sql);

		// wpss_log($response, '-------get_unfnished_folder---response----');
	...
	public function count_complete() {
		...
		$process_table_values = $this->db->get_results("SELECT * FROM {$this->db->base_prefix}wpss_processed_{$this->getTableName()}");
